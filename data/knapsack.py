...
with open('f2_l-d_kp_20_878.txt', 'r') as file:
...
class Island():
    ...
    def truncation_selection(self, size):
        ...
        result.sort(key=lambda k : k.get_fitness())
        # print(result)
        return result[:size]
    ...
    def evolve(self):
        for generation in range(self.num_generations):
            ...
            for island_index, island in enumerate(self.islands):
                ...
                print(f"Best fitness for Island {island_index + 1} in generation {generation + 1}: {1/best_fitness}")
